...
def get_pub(csv_file_path, google_api_client):
	...
	data = pd.read_csv(csv_file_path,header=0, names=['row_id', 'branch_id', 'branch_name', 'holds', 'title', 'author',
       'date', 'url', 'genre', 'branch_name_mod', 'pubdate'])
	data = data[data.title.notnull()]
	data = data[data.branch_name.notnull()]
	data.pubdate = data.pubdate.fillna('NA')
	...
	if os.path.exists('EmergencyDt.csv'):
		...
		suport_dict = {row['title']:row['author'] for index, row in data.iterrows() if row['pubdate']=='NA' and row['title'] not in em_dict.keys()}
		print(len(suport_dict.keys()))
		### Building a dict with the genre and title ###
		genre_dict = em_dict
	else:
		suport_dict = {row['title']:row['author'] for index, row in data.iterrows() if row['pubdate']=='NA'}
		print(len(suport_dict.keys()))
		genre_dict = {}
	...
	if len(suport_dict.keys()) <= 0:
		...
		for key, value in suport_dict.items():
			...
			time.sleep(2)
			try:
				genre_dict[key] = [google_api.list(key,inauthor=value )['items'][0]['volumeInfo']['publishedDate']]
				...
				try:
					for element in google_api.list(key,inauthor=value)['items']: #this approach will try to catch the first result with categorie,
						...
						try:
							genre_dict[key] = [element['volumeInfo']['publishedDate']]
							print('got it on second try, genre: {}'.format(element['volumeInfo']['publishedDate']))
							solved = True
							break
						except KeyError:
							continue
...
get_pub('finalPubDataset.csv', google_api)
